...
まず、新たにリポジトリを作成します。今回は手順を減らすために、リモートリポジトリを先に作ります。
...
もとのファイルにはadd関数があります。
2つの派生ファイルでは、それぞれadd関数の上下に、minus関数とmultiply関数を追加しています。
...
2つの派生ファイルが、それぞれadd関数の上と下という「別の箇所」を変更したと判定されるため、
マージが成功します。

=== コンフリクトが起きる場合

2つの派生ファイルにminus関数とmultiply関数を追加している点は同じですが、
どちらもadd関数の下に追加しています。
...
先述のコンフリクトが起きる場合の例を使います。

minus関数を追加したのがfeatureブランチで、multiply関数を追加したのがmainブランチです。
そして、featureブランチからmainブランチへのプルリクエストが、コンフリクトしてマージできないという状況です。

//image[git-graph-before-merge][コンフリクトしたときの状況]{
//}

準備の手間を省くために、既存のリポジトリをフォークして使います。
...
7個の<がコンフリクトの開始地点で、>がコンフリクトの終了地点です。
