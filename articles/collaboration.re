...
プルリクエストとは、リモートリポジトリのあるブランチを別のブランチにマージしてもらう依頼のことです。
プルリクエストは、GitHubなどのリポジトリのホスティングサービスにある機能です@<fn>{pull-request}。
...
まず新しくリポジトリを作成します。今回は手順を減らすために、リモートリポジトリを先に作ってみます。
...
コンフリクトはgitがマージを自動で行えないときに発生します。
...
この場合、gitはdiff3のアルゴリズムを使って3つのファイルを比較してマージします。
